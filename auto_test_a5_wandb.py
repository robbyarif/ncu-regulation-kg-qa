...
def run_a5_evaluation() -> None:
    ...
    # Initialize wandb
    wandb.init(
        entity="agentic-ai-rob",
        project="ncu-regulation-kg-qa",
        name=f"{llm_loader.MODEL_ID.split('/')[-1]}-{datetime.now().strftime('%Y%m%d-%H%M%S')}",
        config={
            "model_id": llm_loader.MODEL_ID,
            "max_new_tokens": llm_loader.MAX_NEW_TOKENS,
            "retrieval_limit": 15,
            "retrieval_strategy": "hybrid (rules + article content)",
            "security_mode": "dual-layer (keyword + LLM)",
            "total_cases": len(test_cases),
            "start_time": datetime.now().isoformat()
        }
    )
    
    # Wandb table for cases
    cases_table = wandb.Table(columns=["Case ID", "Model", "Type", "Status", "Safety", "Diagnosis", "Answer"])
    total = 0
    passed = 0
    by_type = {
    ...
    for case in test_cases:
        ...
        # Log to wandb table
        cases_table.add_data(
            qid,
            llm_loader.MODEL_ID,
            case["type"],
            "PASS" if ok else "FAIL",
            out.get("safety_decision", "N/A"),
            out.get("diagnosis", "N/A"),
            out.get("answer", "")[:500]
        )
        total += 1
    ...
    wandb.log({
        "metrics/end_to_end_success_rate": passed / total if total else 0,
        "metrics/normal_accuracy": normal_rate,
        "metrics/unsafe_rejection_rate": unsafe_rate,
        "metrics/failure_handling_rate": failure_rate,
        "metrics/diagnosis_label_validity": diagnosis_valid / diagnosis_total if diagnosis_total else 0,
        "scores/task_success_25": pts_task_success,
        "scores/security_validation_15": pts_security,
        "scores/error_detection_8": pts_error_detection,
        "scores/query_regeneration_6": pts_query_regen,
        "scores/repair_resolution_6": pts_repair_resolution,
        "scores/subtotal_60": subtotal_60,
        "results_table": cases_table
    })
    wandb.finish()
